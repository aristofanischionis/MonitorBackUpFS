...
void traverseTrees(Tree **sourceTree, Tree **backupTree, List **sourceINodes,
    ...
    if ((*sourceTree)->root->kid != NULL) {
        while (strcmp((*sourceTree)->root->kid->data.name, (*backupTree)->root->kid->data.name)) {
            ...
            if (firstCase == FILE_IN_SOURCE) { 
                TreeNode *backupKid = addKid((*backupTree)->root, (*sourceTree)->root->kid->data); 
                // make backup path same as source but with different root folder
                char *path = backupPath((*sourceTree)->root->kid->data.path, (*backupTree)->root->data.name); 
                ...
                (*sourceTree)->root->kid->data.inode->copy = backupKid->data.inode;   
            }
            else if (firstCase == DIR_IN_SOURCE) {
                TreeNode *backupSibling = addSiblingSorted((*backupTree)->root->kid, (*sourceTree)->root->kid->data);
            }
            else if (firstCase == FILE_IN_BACKUP) {
                // unlink it in the list of inodes before deleting it from the tree
                deleteINode(backupINodes, (*backupTree)->root->kid->data.inode->inodeNum, (*backupTree)->root->kid->data.name);
                TreeNode * backupPrev = deleteNode(*backupTree, (*backupTree)->root->kid);
            } 
            else if (firstCase == DIR_IN_BACKUP) {
                TreeNode * backupPrev = deleteNode(*backupTree, (*backupTree)->root->kid);
            }
        }
    }
